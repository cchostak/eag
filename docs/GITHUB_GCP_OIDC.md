...
We provide a comprehensive setup script that automates all the steps below:

```bash
# Option 1: GCP Cloud Shell (Recommended)
git clone https://github.com/cchostak/eag.git
cd eag
chmod +x scripts/setup-gcp-oidc.sh
./scripts/setup-gcp-oidc.sh

# Option 2: Local CLI (after gcloud auth login)
cd eag
...
- ✅ Configure GCS bucket for Terraform state
- ✅ Output the GitHub secrets you need to configure

After running the script, add the outputted secrets to your GitHub repository at:
`https://github.com/cchostak/eag/settings/secrets/actions`

**The rest of this document provides manual step-by-step instructions for reference and troubleshooting.**
...
* GitHub repository: we are using `cchostak/eag`
...
gcloud iam workload-identity-pools providers create-oidc github \
  ...
  --attribute-condition="attribute.repository == 'cchostak/eag'"
```
...
gcloud iam service-accounts add-iam-policy-binding \
  ...
  --member="principalSet://iam.googleapis.com/projects/28661575811/locations/global/workloadIdentityPools/github-pool/attribute.repository/cchostak/eag"
```

Result:

* Only `cchostak/eag` can impersonate this service account
